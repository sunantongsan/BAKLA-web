...
const web3Modal = window.Web3Modal.default({
    ...
    providerOptions: {
        ...
        walletconnect: {
            ...
            options: {
                infuraId: 'YOUR_INFURA_ID' // Optional, for fallback RPC
            }
        }
    }
});
...
// ABI and Bytecode (shortened for brevity, full version in previous response)
const contractABI = [/* Full ABI from previous response */];
const contractBytecode = "0x6080604052..."; // Full bytecode from previous response
...
async function connectWallet() {
    try {
        ...
        document.getElementById('account').innerText = 'Connection failed. Please try again.';
    }
}
...
document.getElementById('networkSelect')?.addEventListener('change', async (event) => {
    ...
    if (web3 && selectedAccount) {
        try {
            ...
            document.getElementById('result').innerText = 'Failed to switch network. Please try again.';
        }
    } else {
